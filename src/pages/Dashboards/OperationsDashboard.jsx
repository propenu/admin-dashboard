...
export default function OperationsDashboard({ reportMode = false, businessDevelopmentMode = false }) {
  ...
  const [data, setData] = useState({ projects: {}, properties: {}, leads: [], leadSummary: {}, leadFacets: {}, leadProjects: [], users: [], recent: [], tickets: {} });
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const scopedCreatorIds = applied.role || businessDevelopmentMode
        ? selectedUsers.map((user) => user._id).join(",") || "000000000000000000000000"
        : "";
  ...
  }, [applied, data.users, leadCity, leadLocality, leadPage, leadSource, leadState, leadStatus, projectTypeFilter, recordSearch, reportMode]);
  ...
  return <div className="mx-auto w-full max-w-none space-y-2 pb-4 text-slate-900">
    ...
    {!reportMode ? <>
      ...
      {businessDevelopmentMode && <section className="grid grid-cols-12 gap-2"><Panel title="Project pipeline" subtitle="Project inventory by workflow status" className="col-span-6"><PipelineVisualization rows={projectPipeline} /></Panel><Panel title="Property & plot pipeline" subtitle="Property and plot inventory by workflow status" className="col-span-6"><PipelineVisualization rows={propertyPipeline} /></Panel></section>}
      <section className="grid grid-cols-12 gap-2">
