...
export default function AcAlertsPanel({ alerts = [] }) {
  return (
    <article className="flex h-full min-h-0 flex-col overflow-hidden rounded-[14px] border border-slate-200 bg-white shadow-sm">
      <header className="border-b border-slate-100 px-3.5 py-2.5">
        ...
        <p className="text-[10px] text-slate-500">Collection risks and revenue opportunities</p>
      </header>
      ...
      <div className="flex-1 space-y-2 overflow-y-auto p-3">
        {!alerts.length ? (
          <div className="flex flex-col items-center justify-center gap-2 py-10 text-center">
            ...
            <p className="text-xs font-semibold text-slate-600">Books look healthy</p>
            <p className="text-[10px] text-slate-400">Keep watching failed payments daily.</p>
          </div>
        ) : (
          alerts.map((alert) => {
