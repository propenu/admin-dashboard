...
export default function MhFunnelPanel({ stages = [], summary }) {
  ...
  return (
    <article className="flex h-full min-h-0 flex-col overflow-hidden rounded-[14px] border border-slate-200 bg-white shadow-sm">
      <header className="border-b border-slate-100 px-3.5 py-2.5">
        <h3 className="text-xs font-bold text-slate-900">Lead funnel</h3>
        <p className="text-[10px] text-slate-500">
          Stage volume and drop-off · qualify {summary?.qualificationRate ?? "N/A"}% · booking{" "}
          {summary?.conversionRate ?? "N/A"}%
        </p>
      </header>

      <div className="flex-1 space-y-2 overflow-y-auto p-3">
        {!stages.length ? (
          <p className="py-8 text-center text-xs text-slate-400">No funnel data for this period.</p>
        ) : (
          stages.map((stage, index) => {
            const width = Math.max(12, Math.round((stage.volume / max) * 100));
            return (
                  ...
                  <p className="flex items-center gap-1 text-[10px] text-slate-400">
                    ...
                    {stage.dropOff > 0 && (
                      <span className="text-rose-500">
                        · drop {stage.dropOff} ({stage.dropOffPct}%)
                      </span>
                    )}
                  </p>
                )}
