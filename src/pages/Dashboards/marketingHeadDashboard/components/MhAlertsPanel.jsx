...
export default function MhAlertsPanel({ alerts = [] }) {
  return (
    <article className="flex h-full min-h-0 flex-col overflow-hidden rounded-[14px] border border-slate-200 bg-white shadow-sm">
      <header className="border-b border-slate-100 px-3.5 py-2.5">
        ...
        <p className="text-[10px] text-slate-500">Derived from live acquisition and campaign signals</p>
      </header>
      ...
      <div className="flex-1 space-y-2 overflow-y-auto p-3">
        {!alerts.length ? (
          <div className="flex flex-col items-center justify-center gap-2 py-10 text-center">
            ...
            <p className="text-xs font-semibold text-slate-600">No critical alerts</p>
            <p className="text-[10px] text-slate-400">Keep monitoring CPL and funnel drop-offs.</p>
          </div>
        ) : (
          alerts.map((alert) => {
            ...
            return (
              <div
                key={alert.id}
                className={`rounded-xl border p-2.5 ${ui.wrap}`}
              >
