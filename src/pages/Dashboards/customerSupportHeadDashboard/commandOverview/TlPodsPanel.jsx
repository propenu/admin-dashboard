...
import JourneyMixChart from "../../customerSupportTeamLeadDashboard/commandOverview/JourneyMixChart";
...
function WorkloadBar({ row, maxCases, onSelect, selected }) {
  ...
  return (
    <button
      ...
      className={`grid w-full grid-cols-[110px_1fr] items-center gap-2 rounded-lg px-1 py-0.5 text-left transition sm:grid-cols-[140px_1fr] ${
        selected
    ...
    >
      <div className="min-w-0">
        ...
        <p className="truncate text-[9px] text-slate-400">
          {fmt(row.cceCount)} CCE · {row.isOnline ? "Online" : "Offline"}
        </p>
      </div>
      <div className="min-w-0">
        <div
...
export default function TlPodsPanel({
  pods = [],
  journeyMix = [],
  journeyTotal = 0,
  rangeLabel,
  ...
  const filtered = useMemo(() => {
    ...
    rows.sort((a, b) => {
      ...
      if (sortKey === "cases") return b.totalCases - a.totalCases;
  ...
  return (
    <section className="rounded-2xl border border-slate-200/80 bg-white p-4 shadow-sm">
      <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
        <div>
          <h2 className="text-sm font-black text-slate-950">Team Lead Pods & Analysis</h2>
          <p className="text-[11px] text-slate-500">
            {rangeLabel ? `${rangeLabel} · ` : ""}
            TL workload · CCE process · journey mix · click a pod for detail
          </p>
        </div>
        <button
          type="button"
        ...
        <div className="min-w-0 space-y-3">
          <div className="rounded-xl border border-slate-100 bg-slate-50/60 p-3">
            <div className="mb-2 flex flex-wrap items-center justify-between gap-2">
              <p className="text-[11px] font-bold text-slate-700">TL pod process load</p>
              ...
              <div className="space-y-2">
                {filtered.map((row) => (
                  <WorkloadBar
                    ...
                    selected={String(selectedPodId) === String(row.id)}
                  />
                ))}
              </div>
            ) : (
              <p className="py-6 text-center text-xs text-slate-400">
                No Team Lead pods in this period
              </p>
            )}
          </div>
            ...
            <div className="max-h-[240px] overflow-y-auto overflow-x-hidden [scrollbar-width:thin]">
              <table className="w-full table-fixed text-[11px]">
                ...
                <thead className="sticky top-0 z-[1] bg-white text-[9px] font-bold uppercase tracking-wide text-slate-400">
                  <tr>
                    <th className="px-2.5 py-2 text-left">Team Lead</th>
                    <th className="px-1.5 py-2 text-right">Cases</th>
                    <th className="px-1.5 py-2 text-right">Tickets</th>
                    <th className="px-1.5 py-2 text-right">Stuck</th>
                ...
                <tbody>
                  {filtered.length ? (
                    filtered.map((row) => {
                      const initials = String(row.name || "T")
                        .split(/\s+/)
                        .slice(0, 2)
                        .map((p) => p[0])
                        .join("")
                        .toUpperCase();
                      const selected = String(selectedPodId) === String(row.id);
                      return (
                        <tr
                          key={row.id}
                          ...
                          <td className="px-2.5 py-2 align-middle">
                            <div className="flex min-w-0 items-center gap-1.5">
                              ...
                              <div className="min-w-0">
                                ...
                                <p className="truncate text-[9px] text-slate-400">
                                  {fmt(row.cceCount)} CCE · {fmt(row.openTickets)} tickets
                                </p>
                              </div>
                            </div>
                          </td>
                          <td className="px-1.5 py-2 text-right font-black tabular-nums text-slate-900">
                            {fmt(row.totalCases)}
                          </td>
                          <td className="px-1.5 py-2 text-right font-semibold tabular-nums text-slate-600">
                            {fmt(row.openTickets)}
                          </td>
                          <td className="px-1.5 py-2 text-right font-black tabular-nums text-orange-600">
                            {fmt(row.stuckCases)}
                          </td>
                    ...
                    <tr>
                      <td colSpan={5} className="px-3 py-8 text-center text-slate-400">
                        No Team Leads match this search
                      </td>
                    </tr>
                  )}
                </tbody>
