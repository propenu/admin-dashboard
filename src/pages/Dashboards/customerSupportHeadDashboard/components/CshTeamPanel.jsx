...
import { formatClockTime } from "../customerSupportHeadDashboardData";

export default function CshTeamPanel({ teamMembers = [], onNavigateUsers, onSelectMember }) {
    ...
    <section className="flex h-full min-h-0 min-w-0 flex-col overflow-hidden rounded-[14px] border border-slate-200 bg-white shadow-sm">
      <header className="flex shrink-0 items-center justify-between gap-2.5 border-b border-slate-100 bg-slate-50/80 px-3.5 py-3">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h2 className="text-sm font-bold text-slate-900">Support Team Load</h2>
            <p className="text-[11px] text-slate-500">Executives & leads · live open queue</p>
          </div>
        </div>
        <button
          type="button"
      ...
      <div className="min-h-0 flex-1 overflow-y-auto p-3 [scrollbar-width:thin]">
        {teamMembers.length ? (
          ...
          <p className="px-3 py-10 text-center text-xs text-slate-400">
            No support team members found yet
          </p>
        )}
      </div>
    </section>
