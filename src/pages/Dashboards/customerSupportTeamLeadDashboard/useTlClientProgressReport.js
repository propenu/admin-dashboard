...
import { mapTlClientProgressReport } from "./tlClientProgressReportData";
...
/** Reuse TL team_directory cache — no new backend contract. */
...
export function useTlClientProgressReport(range = {}, filters = {}, options = {}) {
  ...
  const propertiesQuery = useQuery({
    queryKey: ["tl-client-progress-report", "properties", filters],
    queryFn: () =>
      safe(async () => {
  ...
  const projectsQuery = useQuery({
    queryKey: ["tl-client-progress-report", "projects", filters],
    queryFn: () =>
      safe(async () => {
