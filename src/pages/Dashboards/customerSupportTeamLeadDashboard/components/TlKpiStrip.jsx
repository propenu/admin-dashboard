...
import { TL_KPI_QUEUE_TAB } from "../customerSupportTeamLeadDashboardData";
...
const METRICS = [
  ...
  { key: "awaitingCount", label: "Reply pending", icon: MessageSquare, tone: "blue" },
...
export default function TlKpiStrip({
  summary,
  userName,
  onRefresh,
  isFetching,
    ...
    <section className="space-y-2.5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="min-w-0 flex-1">
          ...
          <h1 className="mt-0.5 text-lg font-black leading-tight text-slate-950 sm:text-xl">
            Team floor desk · {userName}
          </h1>
          <p className="mt-1 max-w-2xl text-xs text-slate-500">
            Assign buyer tickets, balance executive load, clear SLA risk, and keep your care pod
            moving.
          </p>
        </div>
      ...
      <div className="grid grid-cols-6 gap-2 sm:gap-3">
        {METRICS.map(({ key, label, icon: Icon, tone }) => {
          const tabKey = TL_KPI_QUEUE_TAB[key];
