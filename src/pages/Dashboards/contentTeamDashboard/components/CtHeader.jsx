import { Download, FilePlus2, Newspaper, RefreshCw } from "lucide-react";
import { DATE_PRESETS, formatRelativeClock } from "../contentTeamDashboardData";
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0 flex-1">
          ...
          <h1 className="mt-0.5 text-lg font-black leading-tight text-slate-950 sm:text-xl">
            Content Command Center · {userName}
          </h1>
          <p className="mt-1 max-w-3xl text-xs text-slate-500">
            Editorial pipeline, publish cadence, engagement, and SEO content that supports Propenu
            property demand.
          </p>
          <p className="mt-1.5 text-[11px] text-slate-400">
            ...
            <span className="mx-1.5 text-slate-300">·</span>
            Today: {summary?.createdToday || 0} created · {summary?.publishedToday || 0} published
          </p>
        </div>
        ...
        trailing={
          <>
            ...
            <strong className="text-slate-700">
              {summary?.avgViews == null ? "N/A" : summary.avgViews}
            </strong>
            <span className="mx-1.5 text-slate-300">·</span>
            Engagement{" "}
            <strong className="text-slate-700">
              {summary?.engRate == null ? "N/A" : `${summary.engRate}%`}
            </strong>
          </>
        }
      />
