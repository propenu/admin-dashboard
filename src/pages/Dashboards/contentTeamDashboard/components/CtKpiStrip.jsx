...
export default function CtKpiStrip({ kpis = [], onMetricClick, activeKey }) {
  if (!kpis.length) {
    return (
      <div className="grid grid-cols-2 gap-2 sm:grid-cols-4 xl:grid-cols-8">
        {Array.from({ length: 8 }).map((_, i) => (
          <div key={i} className="h-16 animate-pulse rounded-[14px] border border-slate-100 bg-slate-50" />
        ))}
      </div>
    );
  }
  ...
  return (
    <div className="grid grid-cols-2 gap-2 sm:grid-cols-4 xl:grid-cols-8">
      {kpis.map((kpi) => {
        const Icon = ICONS[kpi.key] || FileText;
