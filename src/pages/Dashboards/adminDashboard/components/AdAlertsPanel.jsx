...
const severityUi = {
  ...
  low: { icon: AlertTriangle, wrap: "border-slate-200 bg-slate-50", iconWrap: "bg-slate-100 text-slate-600" },
  opportunity: { icon: CheckCircle2, wrap: "border-emerald-200 bg-emerald-50/70", iconWrap: "bg-emerald-100 text-emerald-700" },
};

export default function AdAlertsPanel({ alerts = [], onOpen }) {
  return (
    <article className="flex h-full min-h-0 flex-col overflow-hidden rounded-[14px] border border-slate-200 bg-white shadow-sm">
      <header className="border-b border-slate-100 px-3.5 py-2.5">
        <h3 className="text-xs font-bold text-slate-900">Ops alerts</h3>
        <p className="text-[10px] text-slate-500">What to clear before end of day</p>
      </header>
      <div className="flex-1 space-y-2 overflow-y-auto p-3">
        {!alerts.length ? (
          <div className="flex flex-col items-center justify-center gap-2 py-10 text-center">
            <Lightbulb className="h-8 w-8 text-emerald-400" />
            <p className="text-xs font-semibold text-slate-600">No ops alerts</p>
          </div>
        ) : (
          alerts.map((alert) => {
