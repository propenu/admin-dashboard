...
// MapplsPinMap.jsx — FIXED: onPinChange always read from ref, no stale closures
// Map: Mappls SDK | Reverse geocoding: Nominatim (OpenStreetMap)
import {
  useState,
  useEffect,
  useRef,
...
// ─────────────────────────────────────────────────────────────────────────────
// Mappls SDK loader — singleton promise so the script is loaded only once
// ─────────────────────────────────────────────────────────────────────────────
...
async function reverseGeocode(lat, lng, signal) {
  ...
  return {
    pincode  : a.postcode || "",
    locality : titleCase(stripWard(a.suburb || a.neighbourhood || a.hamlet || a.village || a.town || a.city_district || a.county || "")),
...
// MapplsPinMap
// Props:
//   coordinates  — [lng, lat] | undefined  (controlled from parent)
//   onPinChange  — ({ coordinates, pincode?, locality?, city?, state? }) => void
//
// FIX summary:
//   • Wrapped in memo() so parent re-renders caused by other field changes
//     don't re-render (and re-init) the map.
//   • onPinChange always called via onPinChangeRef so the click handler
//     registered once sees the always-current callback, not a stale closure.
//   • coordinates sync effect keyed on lat/lng primitives, not the array
//     reference, so it doesn't fire when the parent re-renders with same values.
// ─────────────────────────────────────────────────────────────────────────────

const MapplsPinMap = memo(function MapplsPinMap({ coordinates, onPinChange }) {
  const [mapError,     setMapError]     = useState(null);
  const [mapReady,     setMapReady]     = useState(false);
  const [locating,     setLocating]     = useState(false);
  const [markerPlaced, setMarkerPlaced] = useState(false);
  ...
  // Always-fresh ref to onPinChange — the click handler reads from here so it
  // never captures a stale callback even though it's registered only once.
  const onPinChangeRef = useRef(onPinChange);
  useEffect(() => { onPinChangeRef.current = onPinChange; }, [onPinChange]);

  const apiKey = import.meta.env.VITE_MAPPLS_MAP_SDK_KEY;

  // Derive stable lat/lng primitives so the sync effect below doesn't fire on
  // referential inequality of the `coordinates` array.
  const coordLng = coordinates?.[0];
  const coordLat = coordinates?.[1];
  const point = useMemo(() => {
    if (!Number.isFinite(coordLat) || !Number.isFinite(coordLng)) return null;
    return { lat: coordLat, lng: coordLng };
  }, [coordLat, coordLng]);

  // ── Initialise Mappls map (runs once) ─────────────────────────────────────
  useEffect(() => {
    let cancelled = false;
      ...
      try {
        ...
        if (mapRef.current && !clickAttachedRef.current) {
          const handleClick = (event) => {
            ...
            // Place marker immediately for instant feedback
            removeMarker(markerRef.current);
            markerRef.current = new sdk.Marker({
              map      : mapRef.current,
              position : { lat, lng },
  ...
  }, [apiKey, containerId]); // eslint-disable-line react-hooks/exhaustive-deps

  // ── Sync map marker when controlled coordinates change ────────────────────
  // FIX: Dep array uses primitive lat/lng (from useMemo above) instead of the
  //      array reference, so this doesn't fire when parent re-renders with the
  ...
  useEffect(() => {
    ...
    recenterMap(mapRef.current, point.lat, point.lng, MAP_CLICK_ZOOM);
    setMarkerPlaced(true);
  }, [mapReady, point?.lat, point?.lng]); // eslint-disable-line react-hooks/exhaustive-deps
    ...
    <div className="relative h-full w-full">
      ...
      <div className="absolute top-4 left-4 z-[1000] bg-white/90 backdrop-blur-sm rounded-xl shadow-md px-3 py-2 flex items-center gap-2">
        ...
        <div>
          ...
          {Number.isFinite(coordLat) && Number.isFinite(coordLng) && (
            <div className="text-[10px] text-gray-400 mt-0.5">
      ...
      {/* "Your Property" label when pinned */}
      {markerPlaced && (
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-full -mt-8 z-[900] pointer-events-none">
          <div className="bg-white rounded-lg shadow-lg px-3 py-1 border-2 border-[#27AE60]">
            <div className="text-[10px] font-black text-[#27AE60] uppercase tracking-wide">
