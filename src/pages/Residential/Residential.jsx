    ...
    <div className="space-y-4 sm:space-y-6 px-2 sm:px-0">
      ...
      {showDeletePopup && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white w-full max-w-sm md:max-w-md p-4 md:p-6 rounded-xl shadow-xl animate-fade-in">
            <h2 className="text-base md:text-xl font-bold text-red-600 flex items-center gap-2">
              <Trash2 className="w-4 h-4 md:w-5 md:h-5" /> Delete Property
            </h2>
            <p className="text-sm md:text-base text-slate-700 mt-3 leading-relaxed">
              Are you sure you want to delete this residential property? This
              action cannot be undone.
            </p>
            <div className="flex flex-col-reverse sm:flex-row justify-end gap-2 md:gap-3 mt-5 md:mt-6">
              <button
                onClick={() => setShowDeletePopup(false)}
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4">
        <div className="text-center sm:text-left">
          {/* FIX: consistent heading size — base for mobile, 3xl for desktop */}
          <h1 className="text-xl sm:text-2xl lg:text-3xl font-bold text-[#27AE60]">
        ...
        {/* FIX: total count box aligned properly on mobile */}
        <div className="flex flex-row sm:flex-col items-center sm:items-end justify-center gap-2 sm:gap-0 sm:text-right bg-slate-50 sm:bg-transparent rounded-lg sm:rounded-none p-3 sm:p-6">
          <h2 className="text-lg sm:text-xl text-[#27AE60] font-bold">
            {allProperties.total}
          </h2>
          <p className="text-slate-500 text-xs sm:text-sm">Total Properties</p>
        </div>
      </div>

      {/* Stats Grid */}
      {/* FIX: 2-col on mobile, 4-col on large screens */}
      <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        {stats.map((stat, index) => (
          <div
            key={index}
            className="bg-white rounded-xl p-4 sm:p-6 shadow-md hover:shadow-lg transition border cursor-pointer"
            onClick={() => {
              dispatch(setActiveCategory("residential"));
              navigate(stat.route);
            }}
          >
            {/* FIX: icon always left-aligned on all screen sizes */}
            <div className="flex items-center justify-start mb-3 sm:mb-4">
              <div
                className={`w-9 h-9 sm:w-12 sm:h-12 rounded-lg bg-gradient-to-br ${stat.bg} flex items-center justify-center`}
              >
                <stat.icon className={`w-4 h-4 sm:w-6 sm:h-6 ${stat.textColor}`} />
              </div>
            </div>
            {/* FIX: label is consistently small — no oversized text on mobile */}
            <p className="text-slate-700 text-xs sm:text-sm leading-snug">
              {stat.label}
            </p>
            {stat.value !== "" && (
              /* FIX: value is the large number, not the label */
              <p className="text-xl sm:text-2xl font-bold text-[#27AE60] mt-1 sm:mt-2">
                {stat.value}
              </p>
            )}
          </div>
        ...
        {/* Locations Card */}
        <div className="bg-white relative rounded-xl p-4 sm:p-6 shadow-md border">
          <div
            className="flex flex-col w-full gap-2 sm:gap-3 cursor-pointer"
            onClick={() => setOpenLocations(!openLocations)}
          >
            {/* FIX: icon left-aligned to match other cards */}
            <div className="w-9 h-9 sm:w-12 sm:h-12 bg-gradient-to-br from-orange-50 to-orange-100 rounded-lg flex items-center justify-center">
              <MapPin className="w-4 h-4 sm:w-6 sm:h-6 text-[#27AE60]" />
            </div>
            <div className="flex justify-between items-center">
              {/* FIX: consistent text size matching other cards */}
              <p className="text-slate-700 text-xs sm:text-sm leading-snug truncate max-w-[80%]">
                {selectedLocation || "Cities"}
              </p>
              {openLocations ? (
                <ChevronUp className="w-4 h-4 sm:w-5 sm:h-5 text-slate-500 shrink-0" />
              ) : (
                <ChevronDown className="w-4 h-4 sm:w-5 sm:h-5 text-slate-500 shrink-0" />
              )}
            </div>
          </div>
                        ...
                        >
                          {(provided, snapshot) => (
                            ...
                            >
                              ...
                              <span className="bg-[#27AE60] text-white px-2 py-0.5 rounded-full text-[10px] font-bold shrink-0">
                                {loc.count}
                              </span>
                            </div>
                          )}
      ...
      {/* TABS */}
      {/* FIX: tab text, icon and badge sizes consistent across screen sizes */}
      <div className="flex items-center gap-2 sm:gap-6 border-b border-slate-200 overflow-x-auto whitespace-nowrap scrollbar-hide px-1">
        {/* VERIFIED */}
        <button
          onClick={() => setActiveTab("verified")}
          className={`relative flex items-center gap-1.5 sm:gap-2 px-2 pb-3 text-xs sm:text-sm font-medium transition-colors ${
            activeTab === "verified"
              ? "text-green-600"
        ...
        >
          ...
          <span>Verified</span>
          <span className="bg-green-100 text-green-600 px-1.5 sm:px-2 py-0.5 rounded-full text-[10px] sm:text-xs font-semibold">
            {verifiedCount}
          </span>
          {activeTab === "verified" && (
            <span className="absolute left-0 right-0 bottom-0 h-0.5 bg-green-600 rounded-full" />
        ...
        >
          ...
          <span>Pending</span>
          <span className="bg-amber-100 text-amber-600 px-1.5 sm:px-2 py-0.5 rounded-full text-[10px] sm:text-xs font-semibold">
            {pendingCount}
          </span>
          {activeTab === "pending" && (
            <span className="absolute left-0 right-0 bottom-0 h-0.5 bg-amber-600 rounded-full" />
        ...
        >
          ...
          <span>Drafts</span>
          <span className="bg-slate-200 text-slate-700 px-1.5 sm:px-2 py-0.5 rounded-full text-[10px] sm:text-xs font-semibold">
            {draftCount}
          </span>
          {activeTab === "drafts" && (
            <span className="absolute left-0 right-0 bottom-0 h-0.5 bg-slate-600 rounded-full" />
      ...
      <div className="mt-4 sm:mt-6">
        <div className="flex items-center justify-between mb-3 sm:mb-4">
          {/* FIX: heading size scales properly */}
        ...
        {isLoading ? (
          ...
          <div className="bg-slate-50 border-2 border-dashed rounded-2xl py-16 sm:py-20 text-center">
            <Building2 className="w-10 h-10 sm:w-12 sm:h-12 text-slate-300 mx-auto mb-3 sm:mb-4" />
            <p className="text-slate-500 text-sm sm:text-base">No properties found.</p>
          </div>
        ) : (
            ...
            {visibleProperties.map((p) => (
              <div key={p._id} className="relative group">
                ...
                <button
                  ...
                  className="absolute top-3 right-3 p-1.5 sm:p-2 rounded-full bg-red-600 text-white opacity-0 group-hover:opacity-100 transition shadow-lg"
                >
        ...
        >
          {isFetchingNextPage && <LoadingSpinner size="md" />}
          {!hasNextPage && visibleProperties.length > 0 && (
            <p className="text-slate-400 text-xs sm:text-sm">No more properties</p>
          )}
        </div>
      </div>
    </div>
