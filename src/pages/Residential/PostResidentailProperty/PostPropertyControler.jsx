...
export default function PropertyController() {
  ...
  //   const frontendStep = Math.max(backendStep - 1, 0);
  //   console.log("Frontend Step:", frontendStep);
  //   setStep(frontendStep);
  //   setCompletedSteps(Array.from({ length: frontendStep }, (_, i) => i));
  // };
  ...
  return (
    <div className="flex min-h-screen bg-[#f8fffe]">
      {/* ═══ SIDEBAR ═══ */}
      <aside className="hidden lg:flex flex-col w-[350px] max-h-[414px] bg-[FFFFFF]   sticky top-[80px]  h-screen px-6 py-2 shadow-xl rounded-md border border-[#EBECF0] bg-gradient-to-b from-[#E9F7EF] to-[#FFFFFF] ">
        <button
          onClick={handleBack}
          ...
          {STEPS.map((s, i) => {
            ...
            return (
              <div key={i} className="relative ">
                <div
                  className={`relative z-10 w-[80%] flex items-center justify-center  gap-3 px-1 py-2  rounded-xl transition-all duration-200 cursor-default ${
                    isActive
                      ? "bg-gradient-to-r from-[#f0fdf4] to-[#ecfdf5] border border-[#bbf7d0] shadow-2xl"
                      : ""
                  }`}
                >
                  <div
                    className={`w-8 h-8 rounded-xl flex items-center justify-center flex-shrink-0 font-bold text-xs transition-all duration-300 ${
                      isDone
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p
                      className={`text-[10px] mt-0.5 font-medium rounded-2xl w-[fit-content] p-1   ${
                        isActive
                          ? "text-[#27AE60]"
                          : isDone
                ...
                {i < STEPS.length - 1 && (
                  <div
                    className={`absolute left-[18px] z-20 w-1 h-6 ${isDone ? "bg-[#27AE60]" : "bg-[#f0f0f0]"}`}
