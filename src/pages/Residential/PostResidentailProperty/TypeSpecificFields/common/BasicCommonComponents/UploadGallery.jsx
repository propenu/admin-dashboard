...
/* ══════════════════════════════════════════════════════════
   GUARANTEED CANVAS COMPRESSION
   — No library needed
   — Forces JPEG output always
   — Binary searches quality until under TARGET_BYTES
   — Falls back to smaller dimensions if quality alone not enough
══════════════════════════════════════════════════════════ */
  ...
  return new Promise((resolve, reject) => {
    ...
    img.onload = () => {
      ...
      tryNext(0, 0).then((finalBlob) => {
        ...
        console.log(
          `✅ ${file.name}: ${(file.size / 1024).toFixed(0)}KB → ${(finalFile.size / 1024).toFixed(0)}KB`
        );
