...
const FireSafety = ({ error }) => {
  ...
  return (
    <div className="space-y-3">
      <p className="text-xs font-bold text-[#374151] uppercase tracking-wide">Fire Safety</p>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        {FIRE_SAFETY_OPTIONS.map((item) => {
          ...
          return (
            <div
              key={item.key}
              className="flex items-center justify-between px-4 py-3 border-2 border-[#e5e7eb] rounded-xl hover:border-[#bbf7d0] transition-colors"
            >
              ...
              <button
                ...
                role="switch"
                aria-checked={checked}
                onClick={() => updateNestedFieldValue("fireSafety", item.key, !checked)}
