...
export default function Commercial() {
  ...
  const {
    ...
    queryKey: ["commercial"],
    queryFn: ({ pageParam = 1 }) =>
      fetchCommercial({ page: pageParam, limit: 20 }),
  ...
  const deleteMutation = useMutation({
    mutationFn: (id) => deleteCommercial(id),
    onSuccess: () => {
  ...
  const stats = [
    {
      label: "Create Commercial Property",
      icon: Plus,
      value: "",
      route: "/post-property",
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white w-full max-w-sm md:max-w-md p-4 md:p-6 rounded-xl shadow-xl animate-fade-in">
            ...
            <p className="text-sm md:text-base text-slate-700 mt-3 md:mt-2 leading-relaxed">
              Are you sure you want to delete this commercial property? This
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-slate-600 mt-1 max-sm:text-center">
            Explore commercial property listings.
          </p>
        </div>
          ...
          <div className="grid grid-cols-1 gap-6">
            {visibleProperties.map((p) => (
              <div key={p._id} className="relative group">
                <CommercialCard activeTab={activeTab} property={p} />
                <button
                  onClick={() => {
                    setDeleteId(p._id);
