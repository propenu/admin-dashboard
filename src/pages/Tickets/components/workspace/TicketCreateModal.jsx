...
import ColorfulSelect from "../../../Residential/PostResidentailProperty/TypeSpecificFields/common/BasicCommonComponents/ColorfulSelect";
...
export default function TicketCreateModal({
  ...
  const staffRoleOptions = useMemo(() => {
    ...
    return hasAll
      ? options.map((item) =>
          item.value === "all" ? { ...item, label: "All Staff Roles" } : item,
        )
      : [{ label: "All Staff Roles", value: "all" }, ...options];
  }, [roleOptions]);

  const resolvedRequesterRole =
    requesterRole === "staff" && staffRole && staffRole !== "all"
      ? staffRole
      : requesterRole;
      ...
      >
        ...
        <div className="grid min-h-0 flex-1 content-start gap-3 overflow-y-auto p-4 md:grid-cols-2">
          ...
          <div className="md:col-span-2 rounded-2xl border border-slate-200 bg-slate-50/80 p-3">
            ...
            <div className="mt-3 min-h-[130px] max-h-48 overflow-y-auto rounded-xl border-2 border-[#e5e7eb] bg-white shadow-sm">
              {requesterLoading && (
              ...
              {!requesterLoading &&
                visibleRequesterResults.map((user) => {
                  ...
                  const selectedId = String(
                    selectedRequester?._id ||
                      selectedRequester?.userId ||
                  ...
                  return (
                    <button
                      key={`${userId}-${user.role || user.roleName}`}
                      type="button"
                      onClick={() => selectRequester(user)}
                      className={`flex w-full items-center justify-between gap-3 border-b border-[#f5f5f5] px-3 py-2.5 text-left transition last:border-b-0 hover:bg-[#f0fdf4] ${
                    ...
                    >
                      <span className="min-w-0">
                        <span
                          className={`block truncate text-[13px] font-semibold ${
                            isSelected ? "text-[#27AE60]" : "text-slate-900"
                          }`}
                        >
                      ...
                      <span className="flex shrink-0 items-center gap-2">
                        <span className="rounded-full border border-[#27AE60]/30 bg-[#27AE60]/10 px-2 py-1 text-[11px] font-medium text-[#27AE60]">
