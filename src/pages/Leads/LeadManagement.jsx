...
const statuses = ["", "new_lead", "interested", "not_interested", "follow_up", "site_visit", "negotiation", "sale"];
...
export default function LeadManagement() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      } catch (requestError) { if (active) setError(requestError.message || "Unable to load leads"); }
  ...
  const cards = [
    ["Total leads", hasLoaded ? data.summary.total : null, Users, "bg-emerald-500"],
    ["Featured projects", hasLoaded ? data.summary.byCategory.featured || 0 : null, MapPin, "bg-violet-500"],
    ["All properties", hasLoaded ? ["residential", "commercial", "land", "agricultural"].reduce((sum, key) => sum + (data.summary.byCategory[key] || 0), 0) : null, Filter, "bg-blue-500"],
    ["New leads", hasLoaded ? data.summary.byStatus.new_lead || 0 : null, Users, "bg-amber-500"],
  ];
  ...
  return <div className="min-h-full bg-slate-50/70 p-4 sm:p-6">
    <div className="mx-auto max-w-[1600px] space-y-5">
      ...
      <section className="grid grid-cols-2 gap-2 lg:grid-cols-4">{cards.map(([title, value, Icon, color]) => <div key={title} className="rounded-xl border border-slate-100 bg-white px-3 py-2.5 shadow-sm"><div className="flex items-center justify-between gap-2"><div><p className="text-[9px] font-semibold uppercase tracking-wider text-slate-400 sm:text-[10px]">{title}</p>{value === null ? <div className="mt-2 h-5 w-12 animate-pulse rounded bg-slate-100"/> : <p className="mt-1 text-lg font-bold leading-6 text-slate-900 sm:text-xl">{value}</p>}</div><span className={`rounded-lg p-2 text-white ${color}`}><Icon className="h-3.5 w-3.5"/></span></div></div>)}</section>
      <section className="rounded-2xl border border-slate-100 bg-white p-3 shadow-sm">
        ...
        <div className={`${filtersOpen ? "grid" : "hidden"} mt-3 grid-cols-1 gap-2 sm:grid-cols-2 lg:mt-0 lg:grid lg:grid-cols-5 xl:grid-cols-10`}>
          <Select value={filters.category} onChange={(v) => update("category", v)}>{categories.map((v) => <option key={v} value={v}>{label(v)} properties</option>)}</Select>
