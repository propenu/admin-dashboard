...
import { Check, ChevronDown, ChevronUp, MapPin, Save, Search, ShieldCheck, UserRound, Users } from "lucide-react";
import { toast } from "sonner";
import { getAccessRole, getAccessUsers, getPermissionCatalog, updateAccessRolePermissions } from "../../features/accessControl/accessControlService";
...
export default function UserPermissionsPage() {
  ...
  useEffect(() => {
    Promise.all([getAccessUsers(), getPermissionCatalog()])
      .then(([userResult, catalogResult]) => {
  ...
  const roleOptions = useMemo(() => {
    const roles = new Map();
    users.forEach((user) => {
      if (user.roleName) roles.set(user.roleName, String(user.roleName).replace(/_/g, " "));
    });
  ...
  const sharedCount = role ? users.filter((user) => user.roleId === String(role._id)).length : 0;
  ...
  return <div className="mx-auto max-w-[1500px] pb-12 text-slate-900">
    ...
    <div className="grid min-h-[700px] overflow-hidden rounded-[26px] border border-slate-200 bg-white shadow-[0_20px_60px_rgba(15,23,42,0.07)] lg:grid-cols-[340px_minmax(0,1fr)]">
      ...
      <section className="p-5 sm:p-7">
        {!selectedUser ? <div className="grid h-full place-items-center text-center text-slate-500"><div><Users className="mx-auto mb-3" size={42} /><p>Select a dashboard user to inspect access.</p></div></div> : <>
          <div className="flex flex-col gap-4 border-b border-slate-200 pb-5 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-xs font-bold uppercase tracking-wider text-emerald-600">Assigned access</p><h2 className="mt-1 text-2xl font-bold">{selectedUser.name}</h2><p className="mt-1 text-sm text-slate-500">Role: <strong className="text-slate-700">{role?.label || selectedUser.roleName}</strong> · {permissions.size} permissions</p><p className="mt-2 flex items-center gap-1.5 text-sm text-slate-600"><MapPin size={15} className="shrink-0 text-emerald-600" /><span>{location || "Work location not provided"}</span></p></div>{sharedCount > 1 && <div className="max-w-sm rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-xs leading-5 text-amber-900"><strong>Shared role:</strong> Saving changes updates {sharedCount} users assigned to this role.</div>}</div>
