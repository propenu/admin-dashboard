...
import { ArrowLeft, ArrowRight, BadgeCheck, Building2, Check, ChevronDown, KeyRound, Mail, MapPin, ShieldCheck, UserRound } from "lucide-react";
...
export default function CreateCredentialPage() {
  ...
  useEffect(() => {
    getAssignableRoles()
      ...
      .catch((error) => toast.error(error.response?.data?.message || "Unable to load dashboard roles"));
  }, []);
...
function RoleSelect({ roles, value, onChange }) {
  ...
  return <div className="relative z-30">
    ...
    {open && <div className="absolute left-0 right-0 top-[calc(100%+8px)] max-h-64 overflow-y-auto rounded-xl border border-slate-200 bg-white p-1.5 shadow-[0_20px_50px_rgba(15,23,42,0.18)]">
      {roles.length ? roles.map((role) => <button key={role._id} type="button" onClick={() => { onChange(role.name); setOpen(false); }} className={`flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-left text-sm transition ${value === role.name ? "bg-emerald-50 font-bold text-emerald-800" : "text-slate-700 hover:bg-slate-50"}`}><span className="min-w-0 flex-1 truncate">{role.label}</span>{value === role.name && <Check className="shrink-0 text-emerald-600" size={16} />}</button>) : <div className="px-3 py-5 text-center text-sm text-slate-500">No active dashboard roles found</div>}
    </div>}
  </div>;
}
