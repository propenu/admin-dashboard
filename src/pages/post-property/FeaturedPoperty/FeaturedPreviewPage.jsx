  ...
  return (
    <div className="min-h-screen bg-gray-50/60">
      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-4 md:px-6 py-5 sm:py-8 space-y-10 sm:space-y-12">
        ...
        <SectionWrapper
          id="hero" title="Hero Section" subtitle="Branding & primary visual" icon="🏠"
          LeftComponent={LeftEditor}
          RightComponent={HeroSection}
          leftProps={{ formData, setFormData, setLivePreviewData, onSave: handleSave, saving }}
          rightProps={{ data: livePreviewData }}
          registerSectionRef={(el) => (sectionRefs.current["hero"] = el)}
        />

        <SectionWrapper
          id="bhk" title="BHK Configurations" subtitle="Unit types & floor plans" icon="📐"
          LeftComponent={BHKEditor}
          RightComponent={BHKSection}
          leftProps={{ formData, setFormData, onSaveBhk: handleSave, setLivePreviewData, saving, selectedBhkIndex, setSelectedBhkIndex }}
          rightProps={{ data: livePreviewData, selectedBhkIndex, onSelectBhk: setSelectedBhkIndex }}
          registerSectionRef={(el) => (sectionRefs.current["bhk"] = el)}
        />

        <SectionWrapper
          id="amenities" title="Amenities" subtitle="Project facilities & features" icon="✨"
          LeftComponent={AmenitiesEditor}
          RightComponent={AmenitiesSection}
          leftProps={{ formData, setFormData }}
        ...
        <SectionWrapper
          id="gallery" title="Media Gallery" subtitle="Images & videos of the project" icon="🖼️"
          LeftComponent={GalleryEditor}
          RightComponent={Gallery}
          leftProps={{ formData, setFormData, saving, onSave: handleSave, setLivePreviewData }}
          rightProps={{ gallerySummary: livePreviewData.gallerySummary, primaryColor: livePreviewData.color }}
          registerSectionRef={(el) => (sectionRefs.current["gallery"] = el)}
        />

        <SectionWrapper
          id="specifications" title="Specifications" subtitle="Construction & materials" icon="📑"
          LeftComponent={SpecificationEditor}
          RightComponent={Specifications}
          leftProps={{ formData, setFormData, setLivePreviewData, saving, onSave: handleSave }}
          rightProps={{ specifications: livePreviewData.specifications, primaryColor: livePreviewData.color }}
          registerSectionRef={(el) => (sectionRefs.current["specifications"] = el)}
        />

        <SectionWrapper
          id="about" title="About the Project" subtitle="Story, description & highlights" icon="📋"
          LeftComponent={AboutUsEditor}
          RightComponent={AboutUS}
          leftProps={{ formData, setFormData, setLivePreviewData, saving, onSave: handleSave }}
          rightProps={{ aboutSummary: livePreviewData.aboutSummary, primaryColor: livePreviewData.color }}
          registerSectionRef={(el) => (sectionRefs.current["about"] = el)}
        />

        <SectionWrapper
          id="locate" title="Location & Nearby" subtitle="Map & points of interest" icon="📍"
          LeftComponent={LocationEditor}
          RightComponent={LocateUs}
          leftProps={{ formData, setFormData, saving, onSave: handleSave }}
          rightProps={{ data: livePreviewData }}
