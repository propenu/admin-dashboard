...
const GalleryStep = forwardRef(({ payload, update }, ref) => {
  ...
  const handleUpload = (e) => {
    ...
    update({ galleryFiles: [...galleryFiles, ...files] });
    const newSummary = files.map((f, i) => ({
      title: f.name.replace(/\.[^/.]+$/, ""),
      category: "Gallery",
      order: gallerySummary.length + i + 1,
      filename: f.name,
    }));
    update({ gallerySummary: [...gallerySummary, ...newSummary] });
    clr("gallery");
  };
  ...
  return (
    <div className="space-y-6" ref={galleryRef}>
      {/* Upload zone */}
      <label className={`flex flex-col items-center justify-center w-full py-12 px-6 rounded-2xl
      ...
      >
        <div className={`w-16 h-16 rounded-2xl flex items-center justify-center mb-4 transition-all
          ...
          <Upload size={28} className={`${errors.gallery ? "text-red-400" : "text-gray-400 group-hover:text-[#27AE60]"} transition-colors`} />
        </div>
        <p className={`text-sm font-black ${errors.gallery ? "text-red-600" : "text-gray-700 group-hover:text-[#27AE60]"} transition-colors`}>
      ...
      <div className="bg-white border-2 border-gray-200 rounded-2xl p-4">
        <div className="flex items-center justify-between mb-2">
          <span className="text-sm font-black text-gray-700">Upload Progress</span>
          <span className={`text-sm font-black ${count >= 5 ? "text-[#27AE60]" : "text-amber-500"}`}>
        ...
        <div className="h-3 bg-gray-100 rounded-full overflow-hidden">
          <div className="h-full rounded-full transition-all duration-500"
            style={{
              width: `${pct}%`,
              background: count >= 5 ? "linear-gradient(90deg,#27AE60,#2ecc71)" : "linear-gradient(90deg,#f59e0b,#fbbf24)"
            }} />
        </div>
      </div>
      ...
      {count > 0 ? (
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
          {galleryFiles.map((file, i) => (
            <div key={i} className="group relative rounded-2xl overflow-hidden border-2 border-gray-200 shadow-sm aspect-square bg-gray-100">
              ...
              <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-all flex flex-col justify-between p-3">
                ...
                <div>
                  <p className="text-white text-xs font-bold truncate">{gallerySummary[i]?.title}</p>
                  <div className="flex items-center justify-between mt-0.5">
                    <span className="text-white/70 text-[10px]">{gallerySummary[i]?.category}</span>
                    <span className="text-white/70 text-[10px]">#{gallerySummary[i]?.order}</span>
                  </div>
                </div>
              </div>
              {/* Number badge */}
              <div className="absolute top-2 left-2 w-6 h-6 rounded-lg text-white flex items-center justify-center text-[10px] font-black shadow-md"
