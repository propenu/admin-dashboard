...
const SEND_TABS = [
  ...
  { id: "csv",   label: "Bulk CSV Upload", icon: FileSpreadsheet },
];
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      >
        ...
        {file ? (
          <>
            <div className="w-14 h-14 rounded-2xl bg-green-600 flex items-center justify-center shadow-sm shadow-green-200">
              <FileSpreadsheet size={28} className="text-white" />
            </div>
            <div className="text-center">
          ...
          <>
            ...
            <div className="text-center">
              ...
              <p className="text-xs text-gray-400 mt-0.5">or click to browse · .csv and .xlsx supported</p>
            </div>
          </>
        )}
      </div>
      ...
      >
        {busy
          ? <><Loader2 size={16} className="animate-spin" /> Sending Campaign…</>
          : <><Send size={15} /> {file ? "Send Bulk Campaign" : "Upload File First"}</>}
      </button>
    </div>
  );
};
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="flex flex-col gap-2">
        <div className="flex items-center justify-between">
          ...
          {hasFilters && (
            <button
              onClick={() => { setNameSearch(""); setLocality(""); setStateCode(""); setCityName(""); }}
              className="text-xs text-gray-400 hover:text-red-500 flex items-center gap-1 transition-colors"
            >
              <X size={11} /> Clear all
            </button>
        ...
        <div className="grid grid-cols-2 gap-2">
          <div className="relative">
            <Search size={13} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none" />
            <input
              className="w-full pl-8 pr-3 py-2 text-sm border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent placeholder-gray-300 bg-white"
              placeholder="Search by name…"
              value={nameSearch}
              onChange={(e) => setNameSearch(e.target.value)}
            />
          ...
          <div className="relative">
            ...
            />
            <datalist id="locality-options">
              {localityOpts.map((l) => <option key={l} value={l} />)}
            </datalist>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-2">
          <div className="relative">
            <select
              className="w-full appearance-none px-3 py-2 text-sm border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-green-500 bg-white text-gray-700 cursor-pointer pr-8"
              value={stateCode}
              onChange={(e) => setStateCode(e.target.value)}
            >
              <option value="">All States</option>
          ...
          <div className="relative">
            <select
              className="w-full appearance-none px-3 py-2 text-sm border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-green-500 bg-white text-gray-700 cursor-pointer pr-8 disabled:opacity-40 disabled:cursor-not-allowed"
              value={cityName}
              onChange={(e) => setCityName(e.target.value)}
              disabled={!stateCode}
            >
      ...
      <div className="flex flex-col gap-2">
        <div className="flex items-center justify-between">
          <p className="text-xs font-bold uppercase tracking-wide text-gray-500">
            Recipients
            {fetched && (
              <span className="ml-1.5 font-mono text-green-600">{filtered.length} found</span>
            )}
          </p>
          {filtered.length > 0 && (
            <button
                ...
                return (
                  ...
                  >
                    ...
                    <div className={`w-5 h-5 rounded-md border-2 flex items-center justify-center flex-shrink-0 transition-colors ${
                      sel ? "bg-green-600 border-green-600" : "border-gray-300 bg-white"
                    }`}>
                      {sel && <Check size={11} className="text-white" strokeWidth={3} />}
                    </div>
                    {/* Avatar */}
                    <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0 transition-colors ${
                      sel ? "bg-green-600 text-white" : "bg-gray-100 text-gray-500"
                    }`}>
                      {initials}
                    </div>
                    {/* Info */}
                    ...
                    {u.city && (
                      <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-lg flex-shrink-0 ${
                        sel ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-500"
                      }`}>
                        {u.city}
                      </span>
                    )}
    ...
    <Modal title="Send Email Campaign" icon={<Send size={16} />} onClose={onClose}>
      <div className="flex flex-col gap-0">
        ...
        <div className="flex items-center gap-3 p-3 bg-green-50 border border-green-200 rounded-xl mb-5">
          <div className="w-9 h-9 rounded-lg bg-green-600 flex items-center justify-center flex-shrink-0 shadow-sm shadow-green-200">
            <Send size={16} className="text-white" />
          </div>
          <div className="flex-1 min-w-0">
          ...
          <span className="font-mono text-xs text-green-600 bg-white border border-green-200 px-2 py-0.5 rounded-lg flex-shrink-0">
            {item.slug}
          </span>
        </div>
