...
import { Phone, Shield, UsersIcon } from "lucide-react";
...
const BuilderCard = ({ builder, index }) => {
  ...
  const permissions = builder.role?.permissions || [];
...
const Builders = () => {
  ...
  const fetchBuilders = async () => {
    try {
      const response = await getUserSearch("builder");
    ...
    <>
      ...
      <div className="min-h-screen bg-[#f5fcf8] px-8">
          ...
          {!loading && builders.length > 0 && (
            <div className="flex flex-col items-end">
              <span className="text-[36px] font-extrabold text-[#27AE60] leading-none">
                {builders.length.toString().padStart(2, "0")}
              </span>
        ...
        <p className="text-[#000000] text-[13px] ml-[58px] mb-6">
          Manage and view all active builders
        </p>
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {builders.map((builder, idx) => (
              <BuilderCard key={builder._id} builder={builder} index={idx} />
            ))}
          </div>
        )}
      </div>
